...
async def process_mission_file(mission_path):
    ...
    os.remove(f"{current_dir}/Trayectorias/{mission_name}_log.csv")
...
async def monitor_plan_directory():
    ...
    watched_dir = os.path.join(current_dir, "Planes")
    ...
    while True:
        ...
        mission_files = [f for f in os.listdir(watched_dir) if f.endswith('.plan')]
        # Procesar cada archivo que no ha sido procesado aún
        for mission_file in mission_files:
            mission_path = os.path.join(watched_dir, mission_file)
