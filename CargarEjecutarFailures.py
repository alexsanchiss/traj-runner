    ...
    try:
        ...
        with open(csv_filename, mode='w') as csv_file:
            ...
            tasks = [
                ...
                asyncio.create_task(inject_failure_on_cruise(drone, unit_str, f_type_str))
            ]
...
async def inject_failure_on_cruise(drone, unit_str, f_type_str):
    ...
    print(f"-- Inyectando fallo: {unit_str} - {f_type_str}")
    try:
        unit_enum = getattr(FailureUnit, unit_str)
        type_enum = getattr(FailureType, f_type_str)
        # 0 es la instancia por defecto
        await drone.failure.inject(unit_enum, type_enum, 0)
    except Exception as e:
        print(f"Error crítico inyectando fallo: {e}")
